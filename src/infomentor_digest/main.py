...
from .run import Scope, outcome, run
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="run: one run, which sends what changed since the last one. "
        "schedule: run now, then at every RUN_AT time. "
    ...
    parser.add_argument(
        "--sample",
        action="store_true",
        help="report one fact per section and remember none of them",
    )
    parser.add_argument(
...
def dispatch(args: argparse.Namespace) -> int:
    ...
    scope = Scope.SAMPLE if args.sample else Scope.NEW
    print(outcome(run(settings, now().date(), scope=scope, dry_run=args.dry_run)))
    return 0
